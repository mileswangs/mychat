"""
Train a tokenizer using the RustBPE Tokenizers.
...
parser.add_argument(
    "--doc_cap", type=int, default=10_000, help="Maximum characters per document (default: 10,000)"
)
parser.add_argument(
    "--vocab_size", type=int, default=65536, help="Vocabulary size (default: 65536 = 2^16)"
)
...
def text_iterator():
    """
    1) Flatten the batches into a single string iterator
    ...
    for batch in parquets_iter_batched(split="train"):
        for doc in batch:
            ...
            if nchars >= args.max_chars:
                print(f"Reached max chars: {nchars:,}")
                return
...
assert decoded == test_text, "Decoded text does not match original"
...
print(f"Saved token_bytes mapping to {token_bytes_path}")
