...
source = "sft"  # mid | sfr
dtype = "bfloat16"
device_batch_size = 8  # no forward pass will go above this will not OOM
...
save_every = 60  # every how many step to save a checkpoint
...
@torch.no_grad()
def get_batch():
    assistant_end = tokenizer.encode("<|assistant_end|>")  # ok to use this token, it's only for padding and isn't used in the loss.
