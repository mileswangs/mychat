...
# compute/precision
device_type = ""  # cuda|mps|cpu , leave empty for auto-detect
dtype = "bfloat16"
device_batch_size = 4
# optimization
num_epochs = 1
...
wandb_run = DummyWandb() if use_dummy_wandb else wandb.init(project="mychat-sft", name=run, config=user_config, save_code=True)
...
def sft_data_generator(dataset, batch_size):
    ...
    def collecte_and_yield(batch):
        nrows = len(batch)
        ncols = max(len(ids) for ids, mask in batch)  # seq of n creates inputs/targets of n-1
    ...
    while True:
        for i in range(ddp_rank, len(dataset), ddp_world_size):
            ...
            if len(batch) == batch_size:
                yield collecte_and_yield(batch)
                batch = []
